...
// CREATE
app.post('/models', apiLimiter, requireApiKey, upload.single('model'), async (req, res) => {
  if (!req.file) {
...
// LIST
app.get('/models', apiLimiter, requireApiKey, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'An internal server error occurred while fetching models.' });
  }
});

// READ
app.get('/models/:publicId', apiLimiter, requireApiKey, async (req, res) => {
...
// UPDATE
app.put('/models/:publicId', apiLimiter, requireApiKey, upload.single('model'), async (req, res) => {
  const { publicId } = req.params;
  if (!req.file) {
    return res.status(400).json({ error: 'No file uploaded' });
  }
  ...
  try {
    ...
    res.status(500).json({ error: 'An internal server error occurred during update.' });
  } finally {
    try {
      await fs.unlink(req.file.path);
    } catch (unlinkError) {
...
// DELETE
app.delete('/models/:publicId', apiLimiter, requireApiKey, async (req, res) => {
  try {
    const { publicId } = req.params;
    ...
    res.status(500).json({ error: 'An internal server error occurred during deletion.' });
  }
});
